...
# Check bfhd version to decide how to set the client configuration
# the older versions of bfhd accept less arguments
sdk_version=$(bfhd version --long | grep 'cosmos_sdk_version' | awk '{print $2}')
if [[ $sdk_version == *v0.4* ]]; then
	bfhd config chain-id "$CHAINID"
	bfhd config keyring-backend "$KEYRING"
else
	bfhd config set client chain-id "$CHAINID"
	bfhd config set client keyring-backend "$KEYRING"
fi

# Import keys from mnemonics
echo "$VAL_MNEMONIC" | bfhd keys add "$VAL_KEY" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO"

echo "$USER1_MNEMONIC" | bfhd keys add "$USER1_KEY" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO"
echo "$USER2_MNEMONIC" | bfhd keys add "$USER2_KEY" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO"
echo "$USER3_MNEMONIC" | bfhd keys add "$USER3_KEY" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO"
echo "$USER4_MNEMONIC" | bfhd keys add "$USER4_KEY" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO"

# Set moniker and chain-id for Evmos (Moniker can be anything, chain-id must be an integer)
bfhd init "$MONIKER" --chain-id "$CHAINID"
...
bfhd add-genesis-account "$(bfhd keys show "$VAL_KEY" -a --keyring-backend "$KEYRING")" 100000000000000000000000000$BASE_DENOM --keyring-backend "$KEYRING"
bfhd add-genesis-account "$(bfhd keys show "$USER1_KEY" -a --keyring-backend "$KEYRING")" 1000000000000000000000$BASE_DENOM --keyring-backend "$KEYRING"
bfhd add-genesis-account "$(bfhd keys show "$USER2_KEY" -a --keyring-backend "$KEYRING")" 1000000000000000000000$BASE_DENOM --keyring-backend "$KEYRING"
bfhd add-genesis-account "$(bfhd keys show "$USER3_KEY" -a --keyring-backend "$KEYRING")" 1000000000000000000000$BASE_DENOM --keyring-backend "$KEYRING"
bfhd add-genesis-account "$(bfhd keys show "$USER4_KEY" -a --keyring-backend "$KEYRING")" 1000000000000000000000$BASE_DENOM --keyring-backend "$KEYRING"
...
bfhd gentx "$VAL_KEY" 1000000000000000000000$BASE_DENOM --gas-prices ${BASEFEE}$BASE_DENOM --keyring-backend "$KEYRING" --chain-id "$CHAINID"
