...
## default port prefixes for bfhd
NODE_P2P_PORT="2660"
NODE_PORT="2663"
NODE_RPC_PORT="2666"
...
init_func() {
	"$PWD"/build/bfhd keys add $KEY"$i" --keyring-backend test --home "$DATA_DIR$i" --no-backup --algo "eth_secp256k1"
	"$PWD"/build/bfhd init $MONIKER --chain-id $CHAINID --home "$DATA_DIR$i"
	"$PWD"/build/bfhd add-genesis-account \
		"$("$PWD"/build/bfhd keys show "$KEY$i" --keyring-backend test -a --home "$DATA_DIR$i")" 1000000000000000000ubfh,1000000000000000000stake \
		--keyring-backend test --home "$DATA_DIR$i"
	"$PWD"/build/bfhd gentx "$KEY$i" 1000000000000000000stake --chain-id $CHAINID --keyring-backend test --home "$DATA_DIR$i"
	"$PWD"/build/bfhd collect-gentxs --home "$DATA_DIR$i"
	"$PWD"/build/bfhd validate-genesis --home "$DATA_DIR$i"
...
start_func() {
	...
	"$PWD"/build/bfhd start --pruning=nothing --rpc.unsafe \
	...
	EVMOS_PID=$!
	echo "started bfh node, pid=$EVMOS_PID"
	# add PID to array
	arr+=("$EVMOS_PID")
...
if [[ -z $TEST || $TEST == "rpc" || $TEST == "pending" ]]; then
	...
	for i in $(seq 1 "$TEST_QTD"); do
		...
		echo "going to test bfh node $HOST_RPC ..."
