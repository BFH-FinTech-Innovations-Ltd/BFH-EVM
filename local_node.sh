...
LOGLEVEL="info"
# Set dedicated home directory for the bfhd instance
HOMEDIR="$HOME/.tmp-bfhd"
# to trace evm
#TRACE="--trace"
TRACE=""
...
while [[ $# -gt 0 ]]; do
	...
	-n)
		...
		;;
	--no-install)
		echo "Flag --no-install passed -> Skipping installation of the bfhd binary."
		install=false
		shift # Move past the flag
		;;
	*)
...
if [[ $overwrite == "y" || $overwrite == "Y" ]]; then
	...
	# Set client config
	bfhd config set client chain-id "$CHAINID" --home "$HOMEDIR"
	bfhd config set client keyring-backend "$KEYRING" --home "$HOMEDIR"
	...
	echo "$VAL_MNEMONIC" | bfhd keys add "$VAL_KEY" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO" --home "$HOMEDIR"
	echo "$USER1_MNEMONIC" | bfhd keys add "$USER1_KEY" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO" --home "$HOMEDIR"
	echo "$USER2_MNEMONIC" | bfhd keys add "$USER2_KEY" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO" --home "$HOMEDIR"
	echo "$USER3_MNEMONIC" | bfhd keys add "$USER3_KEY" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO" --home "$HOMEDIR"
	echo "$USER4_MNEMONIC" | bfhd keys add "$USER4_KEY" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO" --home "$HOMEDIR"

	# Set moniker and chain-id for Evmos (Moniker can be anything, chain-id must be an integer)
	bfhd init $MONIKER -o --chain-id "$CHAINID" --home "$HOMEDIR"
	...
	bfhd add-genesis-account "$(bfhd keys show "$VAL_KEY" -a --keyring-backend "$KEYRING" --home "$HOMEDIR")" 100000000000000000000000000$BASE_DENOM --keyring-backend "$KEYRING" --home "$HOMEDIR"
	bfhd add-genesis-account "$(bfhd keys show "$USER1_KEY" -a --keyring-backend "$KEYRING" --home "$HOMEDIR")" 1000000000000000000000$BASE_DENOM --keyring-backend "$KEYRING" --home "$HOMEDIR"
	bfhd add-genesis-account "$(bfhd keys show "$USER2_KEY" -a --keyring-backend "$KEYRING" --home "$HOMEDIR")" 1000000000000000000000$BASE_DENOM --keyring-backend "$KEYRING" --home "$HOMEDIR"
	bfhd add-genesis-account "$(bfhd keys show "$USER3_KEY" -a --keyring-backend "$KEYRING" --home "$HOMEDIR")" 1000000000000000000000$BASE_DENOM --keyring-backend "$KEYRING" --home "$HOMEDIR"
	bfhd add-genesis-account "$(bfhd keys show "$USER4_KEY" -a --keyring-backend "$KEYRING" --home "$HOMEDIR")" 1000000000000000000000$BASE_DENOM --keyring-backend "$KEYRING" --home "$HOMEDIR"

	# Sign genesis transaction
	bfhd gentx "$VAL_KEY" 1000000000000000000000$BASE_DENOM --gas-prices ${BASEFEE}$BASE_DENOM --keyring-backend "$KEYRING" --chain-id "$CHAINID" --home "$HOMEDIR"
	## In case you want to create multiple validators at genesis
	## 1. Back to `bfhd keys add` step, init more keys
	## 2. Back to `bfhd add-genesis-account` step, add balance for those
	## 3. Clone this ~/.bfhd home directory into some others, let's say `~/.clonedbfhd`
	## 4. Run `gentx` in each of those folders
	## 5. Copy the `gentx-*` folders under `~/.clonedbfhd/config/gentx/` folders into the original `~/.bfhd/config/gentx`

	# Collect genesis tx
	bfhd collect-gentxs --home "$HOMEDIR"

	# Run this to ensure everything worked and that the genesis file is setup correctly
	bfhd validate-genesis --home "$HOMEDIR"
