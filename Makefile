...
build-docker-goleveldb:
	...
	mkdir -p ./build/.bfhd
	echo '#!/usr/bin/env bash' > ./build/bfhd
	echo "IMAGE_NAME=${DOCKER_IMAGE}:${COMMIT_HASH}" >> ./build/bfhd
	echo 'SCRIPT_PATH=$$(cd $$(dirname $$0) && pwd -P)' >> ./build/bfhd
	echo 'docker run -it --rm -v $${SCRIPT_PATH}/.bfhd:/home/evmos/.bfhd $$IMAGE_NAME bfhd "$$@"' >> ./build/bfhd
	chmod +x ./build/bfhd
